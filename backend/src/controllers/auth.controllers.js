...
export const updateProfile = async (req, res) => {
    try {
        ...
        const userId = req.user._id;    // we are able to do this because we have done "req.user = user" inside auth.middleware.js

        if(!profilePic) {
            return res.status(400).json( {message: "Profile pic is required" });
        }

        const uploadResponse = await cludinary.uploader.upload(profilePic);
